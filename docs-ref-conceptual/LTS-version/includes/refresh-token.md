...
[Refresh tokens in the Microsoft identity platform](/azure/active-directory/develop/refresh-tokens).

Use the [az account get-access-token](/cli/azure/account#az-account-get-access-token) command to
...
- Expiration dates are updated in a format that is supported by [MSAL-based Azure
  CLI](../msal-based-azure-cli.md).
...
- The `expiresOn` property doesn't express "fold" when Daylight Saving Time ends. This can cause
  ...
  "fold", see [PEP 495 – Local Time Disambiguation](https://peps.python.org/pep-0495/).
