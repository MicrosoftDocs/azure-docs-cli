...
[Representational State Transfer (REST) APIs][09] are service endpoints that support different sets
...
[Azure CLI command][03] isn't available.

This article demonstrates the PUT, PATCH, GET, POST, and DELETE HTTP requests to manage Azure
Container Registry resources. The [Azure Container Registry][02] is a managed registry service that
allows you to create and maintain Azure container registries that store container images and related
artifacts.
...
Here's some helpful information when working with [az rest][03]:

- The `az rest` command automatically authenticates using the logged-in credential.
- If Authorization header isn't set, it attaches header `Authorization: Bearer <token>`, where
  `<token>` is retrieved from [Microsoft Entra ID][04].
- The target resource of the token will be derived from the `--url` parameter when the `--url`
...
Sometimes it helps to see an example for a different scenario, so here is an example that uses the
[Microsoft Graph API][06]. To update redirect URIs for an [Application][07], call the
[Update application][05] REST API, as in this code:
...
- [Azure REST API reference][08]
- [az resource][01] command

<!-- link references -->

[01]: ../../docs-ref-autogen/Latest-version/latest/resource.yml
[02]: /azure/container-registry/container-registry-intro
[03]: /cli/azure/reference-index
[04]: /entra/identity/
[05]: /graph/api/application-update?tabs=http
[06]: /graph/api/overview?toc=./ref/toc.json
[07]: /graph/api/resources/application
[08]: /rest/api/azure/
[09]: /rest/api/gettingstarted/#components-of-a-rest-api-requestresponse
