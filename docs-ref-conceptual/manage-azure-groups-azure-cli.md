...
In Azure CLI, use the [az group lock](../latest/docs-ref-autogen/group/lock.yml) commands. For instance, the [az account lock create](/cli/azure/account/lock#az-group-lock-create) command can prevent users from deleting a resource group:
...
To see the current locks on your resource group, use the [az group lock list](/cli/azure/account/lock#az-account-lock-list) command:
