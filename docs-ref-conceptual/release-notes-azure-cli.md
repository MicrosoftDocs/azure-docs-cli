...
* [BREAKING CHANGE] `az sf cluster certificate`: Remove all commands under this group. Please follow the instructions here to add/remove cluster certificates: [Add or remove certificates for a Service Fabric cluster in Azure](/azure/service-fabric/service-fabric-cluster-security-update-certs-azure#add-a-secondary-certificate-using-azure-resource-manager).
...
* There are several changes to `az login`.  (Run `az login --help` for more details.)
  * [BREAKING CHANGE] `~/.azure/accessToken.json` will no longer be created after a successful login. To get an access token, use [`az account get-access-token`](../latest/docs-ref-autogen/account.yml#az_account_get_access_token) instead.
...
* [BREAKING CHANGE] Skip SSL verification via environment `ADAL_PYTHON_SSL_NO_VERIFY` has been removed. See [work behind a proxy](./use-cli-effectively.md#work-behind-a-proxy) for trusting a self-signed root certificate.
