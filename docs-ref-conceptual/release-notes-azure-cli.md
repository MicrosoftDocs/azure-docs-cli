...
* `az netappfiles volume quota-rule create`: Add volume quota rule create command
* `az netappfiles volume quota-rule show`: Add volume quota rule show command
* `az netappfiles volume quota-rule list`: Add volume quota rule list command
* `az netappfiles volume quota-rule update`: Add volume quota rule update command
* `az netappfiles volume quota-rule delete`: Add volume quota rule delete command
...
* Fix #11871: AKV10032: Invalid issuer error for operations in non-default tenant/subscription
...
* `az network vnet-gateway create`: Add new parameter `--nat-rule`
* Add new cmd group `az network vnet-gateway nat-rule`
* `az network vpn-connection create`: Add new parameter `--ingress-nat-rule` and `--egress-nat-rule`
...
* Fix #16203: az webapp log tail supports webapps running on Linux.
...
* `az acr connected-registry`: add support for private preview of connected registry feature.
* `az acr scope-map update`: Deprecated the --add and --remove argument names, replaced with --add-repo and --remove-repo.
* `az acr scope-map create/update`: Introduced gateway permissions to support private preview of connected registry feature.
* `az acr token create`: Introduced synchronization tokens to support private preview of connected registry feature.
...
* `az signalr create`: Add new option `--enable-messaging-logs` for controlling service generate messaging logs or not
* `az signalr update`: Add new option `--enable-messaging-logs` for controlling service generate messaging logs or not
...
* `az sql server audit-policy show`: extend to show servers' audit policy including LA and EH data
...
* `az hdinsight create`: add deprecate information for argument --public-network-access-type and --outbound-public-network-access-type
* `az hdinsight create`: add deprecate information for argument `--public-network-access-type` and `--outbound-public-network-access-type`
...
* `az network application-gateway create`: Fix bug that cannot create without public IP specified
...
* FIX #12871: az aks enable-addons: Autogenerated help example is wrong for virtual-node option
...
* Fix bug while updating flow-log when its subproperty network_watcher_flow_analytics_configuration is None
...
* Add spark job related commands based on track2 sdk
...
* apim api import: support API import and enhance other api level cli commands
...
* Add parameter option list for iot central

### KeyVault

* `az keyvault key encrypt/decrypt`: add parameter `--data-type` for explicitly specifying the type of original data
...
* `az monitor diagnostic-settings subscription`: Support diagnostic settings for subscription
