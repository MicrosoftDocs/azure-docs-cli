...
> - **Description:** Learn the documentation requirements for publishing a new Azure CLI reference group or command
> - **Additional impact:** These standards should idealy be shared with Azure PowerShell
...
The _/_help.py_ file within the Python source code for reference content contains the information that becomes the Azure CLI autogenerated reference docs.  For engineering guideance on in-line help, see [need link here]().
...
1. Provide real-world parameter values.  These examples expect the customer to figure out an acutal value and format for the example.
