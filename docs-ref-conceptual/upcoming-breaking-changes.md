...
- Update the validator in next breaking change release(2.73.0) scheduled for May 2025. the argument `--high-availability` will no longer accept value `SameZone` for new servers created in Business-Critical service tier if the region supports multiple zones.
